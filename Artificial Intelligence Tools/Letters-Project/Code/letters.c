...
//  
//
//  Created by cem on 1/9/19.
//

#include <stdio.h>
#include <math.h>

#define TRAINING_DATA_LENGTH 16000 // for train
#define TEST_DATA_LENGTH 4000 // for test
...
// read train data (plain txt file like train16000.txt) into `train_letters_set[TRAINING_DATA_LENGTH]` in memory
void Read_training_file_from_disk()
{
    FILE *fp;
    int i, j;
    fp = fopen("train16000.txt", "r");
...
// read test data (plain txt file like test4000.txt) into `test_letters_set[TEST_DATA_LENGTH]` in memory
void Read_testing_file_from_disk()
{
    int i, j;
    FILE *fp;
    fp = fopen("test4000.txt", "r");
...
// calculate euclidean distance between a test sample and all training samples
...
int main()
{
    ...
    printf("accuracy: %.3f%% [%f]\n", (float)(right_prediction_count)*100 / TEST_DATA_LENGTH, (float)(right_prediction_count) / TEST_DATA_LENGTH); // Evaluation measure: Classification accuracy - percent of the correctly clas- sified examples in the testing set.
    return 0;
}
